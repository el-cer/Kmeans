...
class Kmeans:
    ...
    def initialize_centroid(self):
        ...
        return centroids
    def model(self,df):
        self.centroid =  self.X.sample(n=self.k) #self.initialize_centroid()
        for _ in tqdm(range(self.nb_test)): 
 
            #permit optimisation of the centroid minimize each centroid per cluster
            dict ={} 
            ...
            self.centroid = self.optimisation(df) #recalcule le centroid par la moyenne des distance de son cluster
            
            #self.plot_clusters(df) #to see each cluster
    ...
    def color_maker(i,df,colors):
        ...
        #this function in fonction of the position of the unique value on a list will return color b or g ...
        return color
